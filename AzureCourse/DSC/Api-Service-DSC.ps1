...
Node $nodeName
  {
	Script InstallNetCore
	{
		...
		SetScript = {
			...
			Write-Host "Deploy PostApi service" -ForegroundColor Green
			$serviceTemp = "$env:TEMP\api-service-" + (Get-Random -Minimum 10000000 -Maximum 99999999)
			$serviceOutput = "C:\www"
			...
			Start-BitsTransfer -Source $using:serviceSourseUrl -Destination $serviceZipOutput
